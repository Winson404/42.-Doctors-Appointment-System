<title>Doctors appointment system | Login History records</title>
<?php
              ...
              <table id="example1" class="table table-bordered table-hover text-sm">
                ...
                <tbody id="users_data">
                  ...
                  if($type_logged_in == 3) {
                    ...
                    $sql = mysqli_query($conn, "SELECT * FROM log_history JOIN users ON log_history.user_Id=users.user_Id WHERE user_type != 3 ORDER BY log_Id DESC");
                  }
                  ...
                  <tr>
                   ...
                   <td>
                      <?php
                          $userTypes = [0 => 'Patient', 1 => 'Secretary', 2 => 'Doctor', 3 => 'Administrator'];
                          $userType = $row['user_type'];
                          $badgeColors = ['primary', 'secondary', 'success', 'danger'];
                          $isValidUserType = ($userType >= 0 && $userType < count($userTypes));
                          $badgeClass = 'badge ' . ($isValidUserType ? 'badge-' . $badgeColors[$userType] : 'badge-light');
                      ?>
                      <span class="<?= $badgeClass ?>"><?= $userTypes[$userType] ?? 'Unknown' ?></span>
                  </td>
